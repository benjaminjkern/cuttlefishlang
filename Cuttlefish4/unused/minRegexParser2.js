...
const getHeuristicObject = (type, rules, heuristics) => {
    ...
    heuristics[type] = { minLength: Number.MAX_VALUE, dict: {}, startTokens: {}, endTokens: {} };
    for (const rule of rules[type]) {
        if (rule.pattern.length < heuristics[type].minLength) heuristics[type].minLength = rule.pattern.length;
        for (const idx in rule.pattern) {
            ...
            if (idx - 0 === 0) {
                if (terminal) heuristics[type].startTokens[token] = true;
                    ...
                    if (token.type !== type) heuristics[type].startTokens = {...heuristics[type].startTokens, ...getHeuristicObject(token.type, rules, heuristics).startTokens };
                }
            }
            if (idx - 0 === rule.pattern.length - 1) {
                if (terminal) heuristics[type].endTokens[token] = true;
                    ...
                    if (token.type !== type) heuristics[type].endTokens = {...heuristics[type].endTokens, ...getHeuristicObject(token.type, rules, heuristics).endTokens };
                }
            }
            if (terminal) heuristics[type].dict[token] = true;
            else {
                heuristics[type].dict["type:" + token.type] = true;
                if (token.type !== type) heuristics[type].dict = {...heuristics[type].dict, ...getHeuristicObject(token.type, rules, heuristics).dict };
            }
        }
    }
...
const matchToType = (type, expression) => {
    ...
    if (!regex.startTokens[forceTerminal(expression[0])]) return debugreturn(false);
    if (!regex.endTokens[forceTerminal(expression[expression.length - 1])]) return debugreturn(false);
    return debugreturn(expression.every(token => regex.dict[forceTerminal(token)]));
}
...
const toString = (obj) => {
    ...
    if (obj.length !== undefined) return '[' + obj.map(toString).join(',') + ']';
...
parseType("N", 'n--nn-n--n-nn-n'.split(''));
