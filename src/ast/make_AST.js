...
const defaultNodeSpecs = {
    ...
    ListType: ["type"],
    Type: ["id"],
    AnonymousSubRoutineGroup: ["statement_block"],
    ...
    Id: ["id"],
    Numlit: ["num"],
    SubRoutine: ["pattern", "guards", "returnTypes", "statements"],
    Operator: ["op"],
    String: ["contents"],
...
function children(node) {
    ...
    return Object.values(node.fields).filter(
        (x) =>
        x && (x.fields || (Array.isArray(x) && x.every((y) => y && y.fields)))
    );
}

function traverse(node, callback, acc, exclude = []) {
    //callback of form f(node,accumulator,type) feel free to append

    var next_acc = callback(node, acc, node.type);
    node
        .children(node)
        .filter((x) => !exclude.includes(x.type))
        .map((x) => {
            if (x.type) {
            ...
            if (Array.isArray(x)) {
                return x
                    .filter((x) => !exclude.includes(x.type))
                    .reduce((racc, y) => {
                        return y.traverse(y, callback, racc, exclude);
                    }, next_acc);
            }
        });
    return acc;
...
function query(node, callback, _1, exclude) {
    //callback of the form f(node,accumulator,type). Returns an object with res(ults) and acc(ulumlator)
