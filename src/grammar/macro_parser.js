...
const macroGrammar = ohm.grammar(fs.readFileSync('src/macro_grammar.ohm'))

module.exports = source => {
    const match = macroGrammar.match(source);
    let context = {
        funcs: {
            get: list => list.reduce((acc, x) => {
                if (acc[x] === undefined) acc[x] = [];
                return acc[x];
            }, context),
            append: list => list.slice(1).map(x => list[0].push(x)),
            print: list => { console.log(list.join(" ")) },
            def: list => {
                let [handle, f] = list;
    ...
    const interpreter = macroGrammar.createSemantics().addOperation('exec', {
        ...
        func(f) { return context.funcs[f.sourceString] },
